...
const studentRegister = async (req, res) => {
  ...
  if (req.session.role != "student")
    ...
    if (!student)
      ...
      res.status(501).json({ success: false, message: "Already booked" });
    } else {
      ...
      if (!bus)
        ...
        if (bus.registered[i].reg != ""){
          res.status(400).json({ success: false, message: "Seat Already Registered" });
      }
        else{
            const registered = bus.registered;
            registered[i] = {reg, seat, name: student.name};
            const modified = await Bus.findOneAndUpdate(
              ...
              logger.info(`Student ${reg} registered for bus ${busId} on seat ${seat}`)
